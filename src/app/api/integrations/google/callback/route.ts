...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (!code || !state) {
    redirect("/?integration_error=google_missing_callback_params");
  }

  const parsedState = parseOAuthState(state);
  const userClient = await createSupabaseServerClient();
  const service = new IntegrationService(userClient, createSupabaseServiceClient());
  await service.completeGoogleCallback(code, state);
  redirect(`/tenants/${parsedState.tenantId}/integrations?connected=google`);
}
